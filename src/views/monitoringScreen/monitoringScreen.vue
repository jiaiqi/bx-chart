  ...
  <div class="contewrap">
    ...
    <el-tabs type="border-card" @tab-click="handleClick">
      ...
      <el-tab-pane label="网络摄像机">
        <surveiVideo :humanFaceData="network"></surveiVideo>
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
  ...
  data () {
    return {
      ...
      CarData: {
        ...
        viewdata: [
          {
            residentNum: '当前出小区车辆', vidilocation: '当前进入小区车辆',
            residentFig: "1547辆", vidilocationFig: "547辆",
          ...
          //     residentNum:'当前出小区车辆',vidilocation:'当前进入小区车辆',
          //     residentFig:"1547辆",vidilocationFig:"547辆",
          //     CamerNumber:"002号",CameraPosition:"小区北门入口",
          //      title:"小区出入车辆抓拍机"
          // }
        ]
      },
  ...
  methods: {
    ...
    async getFacility () { //获取设备
      ...
      if (res.data.state === 'SUCCESS') {
        ...
        for (let j in code) {
          this.getRecord(code[ j ].codes, code[ j ].ip, code[ j ].name, parseInt(j) + 1)

        }
      }
    },
    async getRecord (val, ip, name, indexval) { //获取拍照记录
      ...
      if (res.data.state === 'SUCCESS') {
        ...
        items.forEach((item, index, arr) => {
          if (item.picUri) {
            item.picUri = top.pathConfig.gateway + item.picUri
          }
        });
        let obj = {
          ip: ip,
