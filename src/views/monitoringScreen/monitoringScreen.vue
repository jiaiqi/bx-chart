...
export default {
 ...
 data() {
         ...
         let url = this.getServiceUrl("select", 'srvhk_video_img_select', "zhxq");
            ...
            if (res.data.state === 'SUCCESS'&&res.data.data.length>0) {
                ...
                for(let i in dataItem){
                    ...
                    this.$set(dataItem[i],'eventTime',this.curentTime(dataItem[i].eventTime))
                    // this.curentTime(dataItem[i].eventTime).split(' ')[1]
                console.log(this.curentTime(dataItem[i].eventTime).split(' ')[1])
    ...
    //    this.getFacility('人脸门禁','srvzhxa_person_inout_select','FaceData','getPersonImg',14,'door_code')
     this.getFacility('人脸抓拍','srvzhxa_person_inout_select','humanFaceData','getPersonImg',16,'hk_id')
       console.log(this.curentTime("2020-07-22T18:34:50.000+08:00"),'99999')
    }
  }
</script>
<style lang="scss" >
