  ...
  methods: {
    ...
    async getCarDataList(){ //查询车辆进场信息
        ...
        let itemDataIn = res.data.data
         for(let i in itemDataIn){
           this.$set(itemDataIn[i], "picUri",top.pathConfig.gateway + `/zhxq/hk/getCarImg?picUri=` +itemDataIn[i].vehiclePicUri )
    ...
    async getCarDataListout(val){ //查询车辆出场信息
        ...
        for(let i in itemDataout){
           this.$set(itemDataout[i], "picUri",top.pathConfig.gateway + `/zhxq/hk/getCarImg?picUri=` +itemDataout[i].vehiclePicUri )
        ...
        this.CarData.viewdata.push(obj,obj2)
