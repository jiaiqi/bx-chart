        ...
        <div class="back">
            ...
            <div class="boxImg" v-for="(item,index) in humanFaceData.viewdata" :key="index">
                ...
                <div class="frames">
                    ...
                    <!-- 监控抓拍画面 -->  
                    <div class="photos" :style="[{width:!humanFaceData.ShowMonitoring?'95%':'60%'},{margin:!humanFaceData.ShowMonitoring?'0 auto':' '}]"  v-if="item.monitorMessage&&item.monitorMessage.length>0">
                                <div v-for="(list,i) in item.monitorMessage" :key="i" class="boxPho" >
                                    ...
                                    <div class="fonts">
                                        <div>时间:{{list.eventTime?list.eventTime.split(" ")[1]:''}}</div>
                                        <div v-if="list.typeTitle">{{list.typeTitle}}:{{list.typeIs}}</div>
                                    </div>
                                </div>
                                
                    </div>
                    <div class="photos normalimg" :style="[{width:!humanFaceData.ShowMonitoring?'95%':'60%'},{margin:!humanFaceData.ShowMonitoring?'0 auto':' '}]"  v-else>
                                    暂无抓拍画面
                    </div>
                </div>
            </div>
