...
export default {
  ...
  methods: {
    handleClick (tab) {
      ...
      if (tab.label == '小区车辆抓拍机') {
        ...
        this.getFacility('人脸门禁', 'srvzhxa_person_inout_select', 'FaceData', 'getPersonImg', 16, 'door_code')
      } else if (tab.label == '小区人脸抓拍机') {
        //  获取抓取图片
        //  this.getCaptrueImg(current)
        this.getFacility('人脸抓拍', 'srvzhxa_person_inout_select', 'humanFaceData', 'getPersonImg', 16, 'hk_id')
      } else {
        this.loadingImg = this.normalCar
        this.norText = '暂无可用设备'
      }
    ...
    async getFacility (conName, serName, AlldataArr, mist, rownumber, codeId) { //获取设备
      console.log(mist, rownumber, codeId, '获取设备')
      let url = this.getServiceUrl("select", "srvxqaf_camera_select", "xqaf");
      let req = {
        "serviceName": "srvxqaf_camera_select", "colNames": [ "*" ], "condition": [
    ...
    async getRecord (val, ip, name, indexval, serName, AlldataArr, mist, rownumber) { //获取拍照记录
      ...
      var req = {
        "serviceName": serName, "colNames": [ "*" ], "condition": [
    ...
    async getCaptrueImg (cur, codes, ip, name, indexval) {
        ...
        "condition": [
          {
            "colName": "startTime",
            "ruleType": "eq",
            "value": this.getPreDate(this.curentTime()) + " " + this.curentTime().split(' ')[ 1 ]
          },
          {
            "colName": "endTime",
            "ruleType": "eq",
      ...
      if (res.data.state === 'SUCCESS' && res.data.data.length > 0) {
        ...
        for (let i in dataItem) {
          ...
          this.$set(dataItem[ i ], 'eventTime', this.curentTime(dataItem[ i ].eventTime).split(' ')[ 1 ])

          // this.curentTime(dataItem[i].eventTime).split(' ')[1]
        }
        let obj = {
          ip: ip,
          name: name,
  ...
  created () {
    ...
    // console.log(this.getPreDate(this.curentTime())+" " + this.curentTime().split(' ')[1])
    console.log(this.curentTime('2020-07-23T01:24:15.000+08:00').split(' ')[ 1 ], '***----------****')
  }
}
</script>
<style lang="scss" >
